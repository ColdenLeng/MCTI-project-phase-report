...
def main():
    ...
    train_loader = loaders['shadow_train']
    test_loader  = loaders['shadow_test']

    model = SmallCNN(num_classes=10).to(device)
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(model.parameters(), lr=1e-3)

    result_dir = '../results/stl10/shadow'
    os.makedirs(result_dir, exist_ok=True)

    model_path = os.path.join(result_dir, 'shadow_model.pth')
    log_path = os.path.join(result_dir, 'shadow_log.txt')
    ...
    print(f"Shadow model saved to {model_path}")
