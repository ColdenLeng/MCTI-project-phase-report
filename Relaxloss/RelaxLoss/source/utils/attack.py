...
class BaseAttacker(object):
    ...
    def run_blackbox_attacks(self):
        ...
        #
        s_features_pos = np.stack([bb_benchmark.s_tr_loss, bb_benchmark.s_tr_conf, bb_benchmark.s_tr_entr], axis=1)
        s_features_neg = np.stack([bb_benchmark.s_te_loss, bb_benchmark.s_te_conf, bb_benchmark.s_te_entr], axis=1)
        t_features_pos = np.stack([bb_benchmark.t_tr_loss, bb_benchmark.t_tr_conf, bb_benchmark.t_tr_entr], axis=1)
        t_features_neg = np.stack([bb_benchmark.t_te_loss, bb_benchmark.t_te_conf, bb_benchmark.t_te_entr], axis=1)
        #
    ...
    def gradient_based_attack_wrt_x(self, dataloader, model):
        ...
        for batch_idx, (inputs, targets) in enumerate(tqdm(dataloader)):
            ...
            for input, target in zip(inputs, targets):
                ...
                output = model(input)
                loss = self.crossentropy(output, torch.unsqueeze(target.long(), 0))
    ...
    def get_blackbox_statistics(self, dataloader, model):
        print("loader len:", len(dataloader))
        count = 0
        for batch_idx, (inputs, targets) in enumerate(dataloader):
            count += 1
        print("Total batch count:", count)
        ...
        if len(logits) == 0:
            print("ERROR: logits is empty! DataLoader has no data.")
        else:
