...
def main():
    ...
    model = models.MIMICClassifier(input_size=8282, num_classes=100)
    ...
    if args.if_onlyeval:
        print('\nEvaluation only')
        test_loss, test_acc, y_true, y_score = trainer.test(model)
        auroc_macro = roc_auc_score(y_true, y_score, multi_class='ovr', average='macro')
        print(f"Test AUROC (macro avg): {auroc_macro:.4f}")
        print("[DEBUG] AUROC y_true classes:", np.unique(y_true, return_counts=True))
        print("Debug: y_true and y_score received")
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        test_loss, test_acc, test_acc5, test_auroc = trainer.test(model)
