...
def parse_arguments():
    ...
    parser.add_argument('--dataset', type=str, choices=['CIFAR10', 'CIFAR100','STL10'], help='dataset name')
...
def main():
    ...
    logger.set_names(['LR', 'Train CE', 'Test CE', 'Train Acc', 'Test Acc', 'Test Acc5'])
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        print('[DEBUG] test() returns:', trainer.test(model))
        test_result = trainer.test(model)
        print('[DEBUG] test() returns:', test_result)
        print('[DEBUG] type:', type(test_result))
        test_loss, test_acc, test_acc5, test_auroc = trainer.test(model)

        for param_group in optimizer.param_groups:
            lr = param_group['lr']
        logger.append([lr, train_loss, test_loss, train_acc, test_acc, test_acc5])
        print('Epoch %d, Train acc: %.2f, Test acc: %.2f, AUROC: %.4f, lr: %.4f' %
              (epoch, train_acc, test_acc, test_auroc, lr))
