...
class Attacker(BaseAttacker):
    def set_dataloader(self):
        ...
        if self.args.dataset == 'CIFAR10':
            ...
            self.dataset = STL10
            self.num_classes = 10
            self.dataset_size = 13000  
        else:
            raise ValueError('Dataset not supported: %s' % self.args.dataset)
        ...
        print('[DEBUG] load full_idx.npy path:', os.path.join(self.args.target_path, 'full_idx.npy'))
        print('[DEBUG] indices length:', len(indices))
        print('[DEBUG] indices (head):', indices[:20])
        ...
        if os.path.exists(os.path.join(self.args.shadow_path, 'full_idx.npy')):
            ...
            print('[DEBUG] shadow index full_idx.npy length:', len(shadow_indices))
            print('[DEBUG] shadow index full_idx.npy head:', shadow_indices[:20])
            assert np.array_equiv(indices, shadow_indices)
            
        self.partition = Partition(
            dataset_size=5000,
            indices=indices,
            target_train_size=4500,   
            target_test_size=200,
            shadow_train_size=200,    
            shadow_test_size=100,
            pseudo_test_size=0
        )
        ...
        print('[DEBUG] target_train_idx:', len(target_train_idx))
        print('[DEBUG] target_test_idx:', len(target_test_idx))
        print('[DEBUG] shadow_train_idx:', len(shadow_train_idx))
        print('[DEBUG] shadow_test_idx:', len(shadow_test_idx))
