...
from utils import mkdir, str2bool, write_yaml, load_yaml, adjust_learning_rate, \
    AverageMeter, Bar, accuracy, accuracy_binary, one_hot_embedding, CrossEntropy_soft
...
def parse_arguments():
    ...
    parser.add_argument('--binary', type=str2bool, default=False, help='Use binary classification with BCE')
    return parser
...
class Trainer(NonImageTrainer):
    def set_criterion(self):
        self.binary = self.args.binary
        self.crossentropy_noreduce = nn.BCEWithLogitsLoss(reduction='none') if self.binary else nn.CrossEntropyLoss(reduction='none')
        self.crossentropy_soft = CrossEntropy_soft
        self.crossentropy = nn.BCEWithLogitsLoss() if self.binary else nn.CrossEntropyLoss()
        self.softmax = nn.Sigmoid() if self.binary else nn.Softmax(dim=1)
        self.alpha = self.args.alpha
        self.upper = self.args.upper

    def train(self, model, optimizer, epoch, binary=False):
        model.train()
        losses = AverageMeter()
        ...
        for batch_idx, (inputs, targets) in enumerate(self.trainloader):
            ...
            if epoch % 2 == 0:
                ...
                if loss_ce > self.alpha:
                    ...
                    if self.binary:
                        ...
                        confidence = torch.clamp(confidence, min=0., max=self.upper)
                        soft_targets = confidence.detach()
                        loss = self.crossentropy(outputs, soft_targets)
                    else:
            ...
            if self.binary:
                prec1 = accuracy_binary(torch.sigmoid(outputs), targets)
                prec5 = torch.tensor(0.0)
            else:
...
def main():
    ...
    criterion = nn.BCEWithLogitsLoss() if args.binary else nn.CrossEntropyLoss()
    optimizer = optim.SGD(model.parameters(), lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay)

    start_epoch = 0
    logger = trainer.logger
    logger.set_names(['lr', 'train_loss', 'test_loss', 'train_acc', 'test_acc', 'train_acc5', 'test_acc5'])
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        train_ce, train_acc, train_acc5 = trainer.train(model, optimizer, epoch, binary=args.binary)
        test_ce, test_acc = trainer.test(model, criterion)
        test_acc5 = 0.0
        test_auc = 0.0
        ...
        logger.append([
            to_item(lr),
            to_item(train_ce), to_item(test_ce),
            to_item(train_acc), to_item(test_acc),
            to_item(train_acc5), to_item(test_acc5)
        ])
