...
from dataset import prepare_texas, prepare_purchase, prepare_covertype, prepare_adult
...
def parse_arguments():
    ...
    parser.add_argument('--binary', type=str2bool, default=False,
                    help='Use BCEWithLogitsLoss for binary classification')
    return parser
...
class Attacker(BaseAttacker):
    def set_dataloader(self):
        ...
        elif self.args.dataset == 'Covertype':
            target_train, target_test, shadow_train, shadow_test, pseudo_attack = prepare_covertype(self.args.random_seed)
            self.num_classes = 7

        elif self.args.dataset == 'Adult':
            target_train, target_test, shadow_train, shadow_test, pseudo_attack = prepare_adult(self.args.random_seed)
            self.num_classes = 2
