...
class Benchmark(object):
    ...
    def _mem_inf_thre_perclass(self, metric_name, s_tr_values, s_te_values, t_tr_values, t_te_values):
        labels_tr = self.s_tr_labels.squeeze() if self.s_tr_labels.ndim > 1 else self.s_tr_labels
        labels_te = self.s_te_labels.squeeze() if self.s_te_labels.ndim > 1 else self.s_te_labels
        labels_tt = self.t_tr_labels.squeeze() if self.t_tr_labels.ndim > 1 else self.t_tr_labels
        labels_te2 = self.t_te_labels.squeeze() if self.t_te_labels.ndim > 1 else self.t_te_labels
    
        total_mem, total_nonmem = 0, 0
        t_mem_correct, t_nonmem_correct = 0, 0
    
        info = {}
        result = {}
    
        for num in range(self.num_classes):
            thre = self._thre_setting(s_tr_values[labels_tr == num], s_te_values[labels_te == num])
    
            t_tr_mem = np.sum(t_tr_values[labels_tt == num] >= thre)
            t_tr_total = np.sum(labels_tt == num)
    
            t_te_mem = np.sum(t_te_values[labels_te2 == num] < thre)
            t_te_total = np.sum(labels_te2 == num)

            total_mem += t_tr_total
            total_nonmem += t_te_total

            t_mem_correct += t_tr_mem
            t_nonmem_correct += t_te_mem

        attack_acc = 100.0 * (t_mem_correct + t_nonmem_correct) / (total_mem + total_nonmem + 1e-8)

        info['attack_acc'] = attack_acc
        info['t_mem_correct'] = t_mem_correct
        info['t_nonmem_correct'] = t_nonmem_correct
        info['t_total_mem'] = total_mem
        info['t_total_nonmem'] = total_nonmem
...
class Benchmark_Blackbox(Benchmark):
    def compute_bb_scores(self):
        ...
        # convert labels to integer if binary
        if self.s_tr_labels.dtype != np.int64:
        ...
        if self.s_tr_outputs.ndim == 2 and self.s_tr_outputs.shape[1] > 1:
            ...
            self.s_tr_conf = self.s_tr_outputs.squeeze()
            self.s_te_conf = self.s_te_outputs.squeeze()
            self.t_tr_conf = self.t_tr_outputs.squeeze()
            self.t_te_conf = self.t_te_outputs.squeeze()
    ...
    def _m_entr_comp(self, probs, true_labels):
        ...
        if probs.shape[1] == 1:
        # Binary case: modified entropy fallback = standard entropy
            return self._entr_comp(probs)
...
class BaseAttacker(object):
    ...
    def run_nn_attack(self, s_logits_pos, s_logits_neg, t_logits_pos, t_logits_neg, if_load_checkpoint=True):
        ...
        if os.path.exists(os.path.join(checkpoint_dir, 'attack_model.pt')) and if_load_checkpoint:
            ...
            max_epoch = 200
            lr = 0.001
            input_dim = s_logits_pos.shape[1] if len(s_logits_pos.shape) > 1 else 1
            attack_model = NNAttack(input_dim=input_dim)
...
class NNAttackTrainer(BaseTrainer):
    ...
    def construct_dataloader(stat_pos, stat_neg):
        ...
        attack_targets = attack_targets.astype(int)
