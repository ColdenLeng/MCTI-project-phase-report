...
def parse_arguments():
    ...
    parser.add_argument('--dataset', type=str, choices=['Texas', 'Purchase', 'MIMIC'], help='dataset name')
...
class Trainer(NonImageTrainer):
    def set_criterion(self):
        ...
        # === Load y from mimic_data.npz ===
        try:
            y = np.load("data/MIMIC/filtered_labels_top100.npy")
        except Exception as e:
            print("[Warning] Could not load filtered labels. Using uniform class weights instead.")
            y = None
        ...
        if y is not None:
            ...
            print(f"[INFO] Loaded class weights: {weights.shape}")
            self.crossentropy_noreduce = nn.CrossEntropyLoss(weight=weights, reduction='none')
            #
            self.crossentropy = nn.CrossEntropyLoss(weight=weights)
            #
        else:
            self.crossentropy_noreduce = nn.CrossEntropyLoss(reduction='none')
            #
    ...
    def train(self, model, optimizer, epoch):
        ...
        for batch_idx, (inputs, targets) in enumerate(self.trainloader):
            ...
            top_k_vals = (1, 5) if self.num_classes >= 5 else (1,)
            prec1, *rest = accuracy(outputs.data, targets.data, topk=top_k_vals)
            prec5 = rest[0] if rest else 0.0
...
def main():
    ...
    logger.set_names(['lr', 'train_loss', 'test_loss', 'train_acc', 'test_acc', 'train_acc5', 'test_acc5', 'test_auroc'])
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        test_ce, test_acc, test_acc5, auroc = trainer.test(model)
        for param_group in optimizer.param_groups:
            lr = param_group['lr']
        logger.append([lr, train_ce, test_ce, train_acc, test_acc, train_acc5, test_acc5, auroc])
