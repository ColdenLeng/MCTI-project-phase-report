...
class Trainer(NonImageTrainer):
    def set_criterion(self):
        ...
        labels_list = [label.item() for _, label in self.trainset]
        label_counts = Counter(labels_list)
        total_count = sum(label_counts.values())
        class_weights = [total_count / label_counts[i] for i in range(self.num_classes)]
        class_weights = torch.tensor(class_weights, dtype=torch.float32).to(self.device)
        
        self.crossentropy_noreduce = nn.CrossEntropyLoss(reduction='none', weight=class_weights)
